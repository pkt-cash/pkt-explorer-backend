...
const types = module.exports.types = (Object.freeze((() => {
  ...
  _.FixedString = (num /*:number*/) => {
    return { _flow: 'string', ch: `FixedString(${String(num)})` };
  };
  _.String = { _flow: 'string', ch: 'String' };
  _.DateTime = (tz /*:string*/) => {
    return { _flow: 'Date', ch: `DateTime('${tz}')` };
  };
  _.DateTime_number = (tz /*:string*/) => {
    return { _flow: 'number', ch: `DateTime('${tz}')` };
  };
  ...
  for (let n = 8; n <= 64; n *= 2) {
    _['Int' + n] = { _flow: 'number', ch: 'Int' + n };
    _['UInt' + n] = { _flow: 'number', ch: 'UInt' + n };
    _['UInt' + n + '_string'] = { _flow: 'string', ch: 'UInt' + n };
    _['Int' + n + '_string'] = { _flow: 'string', ch: 'UInt' + n };
    if (n >= 32) {
      _['Float' + n] = { _flow: 'number', ch: 'Float' + n };
    }
  }
  return _;
})())/*:ClickHouseTypes_t*/);
...
const mergeUpdate = /*::<X,Y>*/(
  ...
  ch.withTempTable(tempTableProto, newData, (ch, tempTable, done) => {
    ch.modify(`INSERT INTO ${outTable.name()}
      SELECT
        ${Object.keys(outTable.fields()).map((f) => {
      ...
      ANY RIGHT JOIN ${tempTable.name()} ON
        ${keyFieldNames.map((f) => (
